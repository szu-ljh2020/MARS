...
class GraphTransformer_CatEmd(nn.Module):
    def __init__(self, encoder, src_embed, generator):
        super(GraphTransformer_CatEmd, self).__init__()
        self.encoder = encoder
        self.src_embed = src_embed
        self.generator = generator
    ...
    def predict(self, out, out_mask):
        return out, self.generator(out, out_mask)
...
def attention(query, key, value, query2, key2, adj_matrix, distances_matrix, edges_att,
    ...
    scores = torch.matmul(query, key.transpose(-2, -1)) \
             / math.sqrt(d_k)
    scores_local = torch.matmul(query2, key2.transpose(-2, -1)) \
                   / math.sqrt(d_k)
    if mask is not None:
        scores = scores.masked_fill(mask.unsqueeze(1).repeat(1, query.shape[1], query.shape[2], 1) == 0, -inf)
        scores_local = scores_local.masked_fill(mask.unsqueeze(1).repeat(1, query2.shape[1], query2.shape[2], 1) == 0,
                                                -inf)
    ...
    # mask none neighbours, to compute softmax prob attention
    scores_local = scores_local.masked_fill(adj_matrix == 0, -inf)
    p_adj = F.softmax(scores_local, dim=-1)
    p_adj = adj_matrix * p_adj
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, adj_matrix, distances_matrix, edges_att, mask=None):
        ...
        query, key, value, query2, key2 = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value, query, key))]
        ...
        x, self.attn, self.self_attn = attention(query, key, value, query2, key2, adj_matrix,
                                                 p_dist, edges_att,
...
class MATNet_aug(nn.Module):
    def __init__(self, d_atom, N=2, d_model=128, h=8, dropout=0.1,
                 ...
                 control_edges=False, integrated_distances=False,
                 scale_norm=False, init_type='uniform', use_adapter=False, n_generator_layers=1):
        super(MATNet_aug, self).__init__()
        self.mat = make_model(d_atom, N, d_model, h, dropout,
                              lambda_attention, lambda_distance, trainable_lambda,
                              N_dense, leaky_relu_slope, aggregation_type,
                              dense_output_nonlinearity, distance_matrix_kernel,
                              use_edge_features, n_output,
                              control_edges, integrated_distances,
                              scale_norm, init_type, use_adapter, n_generator_layers)

    def forward(self, node_feature, batch_mask, adj_m, dist_m, edges_att):
        h = self.mat(node_feature, batch_mask, adj_m, dist_m, edges_att)
        h = F.sigmoid(h)
        return h


class MATNet_aug_CatGraph(nn.Module):
    def __init__(self, d_atom, N=2, d_model=128, h=8, dropout=0.1,
                 lambda_attention=0.3, lambda_distance=0.3, trainable_lambda=False,
                 N_dense=2, leaky_relu_slope=0.0, aggregation_type='mean',
                 dense_output_nonlinearity='relu', distance_matrix_kernel='softmax',
                 ...
                 control_edges=False, integrated_distances=False,
                 scale_norm=False, init_type='uniform', use_adapter=False, n_generator_layers=1):
        super(MATNet_aug_CatGraph, self).__init__()
        c = copy.deepcopy
        attn = MultiHeadedAttention(h, d_model, dropout, lambda_attention, lambda_distance, trainable_lambda,
                                    distance_matrix_kernel, use_edge_features, control_edges, integrated_distances)
        ff = PositionwiseFeedForward(d_model, N_dense, dropout, leaky_relu_slope, dense_output_nonlinearity)
        model = GraphTransformer_CatEmd(
            Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout, scale_norm, use_adapter), N, scale_norm),
            Embeddings(d_model, d_atom, dropout),
            Generator(d_model, 'mean', d_model, n_generator_layers, leaky_relu_slope, dropout, scale_norm))
        ...
        for p in model.parameters():
            if p.dim() > 1:
                ...
                #     xavier_uniform_small_init_(p)
        self.mat = model
        self.node_linear = nn.Linear(d_model, d_model)
        self.g_node_linear = nn.Linear(d_model, n_output)

    def forward(self, node_feature, batch_mask, adj_m, dist_m, edges_att):
        h, e = self.mat(node_feature, batch_mask, adj_m, dist_m, edges_att)
        h = self.node_linear(h)
        e = e.unsqueeze(1).repeat(1, h.shape[1], 1)
        h = F.sigmoid(self.g_node_linear(F.relu(h + e)))
        return h
